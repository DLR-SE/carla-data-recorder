...
class IMUDataWriter(ParquetWriterMixin, DataWriter):

    def __init__(self, imu_queue: Queue,
                 recording_dir: Union[str, Path], sensor_id: str, output_format: str,
                 **output_format_kwargs: Any):
        """
        Writes imu data to the disk.

        Args:
            imu_queue (Queue): a queue that receives CARLA's imu data
        ...
        """
        super().__init__(recording_dir, sensor_id, output_format, output_format_kwargs, main_queue=imu_queue)
    ...
    def _work_iteration(self):
        ...
        imu_data = {
            ...
            'acceleration': queue_items['acceleration'],
            'angular_velocity': queue_items['angular_velocity']
        }

        if self.output_format == 'json':
            with open(os.path.join(self.data_dir, f'{frame_id:06d}.json'), 'wt') as file:
                json.dump(imu_data, file, default=customs_to_json)
        elif self.output_format == 'parquet':
            self.write_to_parquet(frame_id, imu_data)
        else:
    ...
    def get_data_directory(cls, recording_dir: Path) -> Path:
        return recording_dir / 'sensor' / 'imu'
    ...
    def get_parquet_schema(cls) -> pa.Schema:
        return pa.schema([('timestamp', pa.float32()),
                          ('acceleration', pa.list_(pa.float32())), ('angular_velocity', pa.list_(pa.float32()))])
