...
#
# This file is mainly based on work covered by the following copyright and
# permission notice:
#
...
##################################################################################
# CUSTOM IMPORT OF CARLA DATA RECORDER
...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        if Version(dist.version) < Version(VERSION):
            raise ImportError(f"CARLA version {VERSION} or newer required. CARLA version found: {dist}")
        ...
        ##################################################################################
        # MODIFIED BY INSTANTIATING CARLA DATA RECORDER
        if args.cdr_enable:
            if args.cdr_results_dir is None:
                raise ValueError('CARLA Data Recorder was requested but no results directory was specified via '
                                 '`--cdr_results_dir`.')
            if args.cdr_config is not None:
                self.cdr = CARLADataRecorder.from_config(args.cdr_results_dir, self.client, args.cdr_config)
            else:
                # use default config for other parameters than the CARLA Client connection
                self.cdr = CARLADataRecorder(args.cdr_results_dir, self.client,
                                             host=args.host, port=int(args.port), timeout=self.client_timeout)
        else:
            self.cdr = NoRecorder()
        ##################################################################################
    ...
    def _load_and_run_scenario(self, config):
        ...
        try:
            ...
            ##################################################################################
            # MODIFIED BY USING CARLA DATA RECORDER
            with self.cdr(self.world):
    ...
    def _run_scenarios(self):
        ...
        #########################################################################################################
        # MODIFIED BY USING CARLA DATA RECORDER
        num_sim_runs = len(scenario_configurations) * self._args.repetitions
        self.cdr.set_num_simulation_runs(num_sim_runs)
        #########################################################################################################
...
def main():
    ...
    ##################################################################################
    # CUSTOM ARGS FOR CARLA DATA RECORDER
    parser.add_argument('--cdr_enable', action='store_true',
                        help='CDR: Enables the data recording using the CARLA Data Recorder. Output directory must '
                        'be specified via `--cdr_results_dir`.')
    parser.add_argument('--cdr_results_dir', type=str, default=None,
                        help='CDR: The directory where simulation results shall be stored.')
    parser.add_argument('--cdr_config', type=str, default=None,
                        help='CDR: Path to a configuration file that shall be used for the CARLA Data Recorder.')
    ##################################################################################
